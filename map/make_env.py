...
def after_install(options, home_dir):
	...
	if not os.path.exists(etc):
		os.makedirs(etc)
	subprocess.call([join(home_dir, 'bin', 'pip'), 'install', '--upgrade', 'setuptools'])
	if commands.getoutput('%s help install | grep "\-\-allow\-all\-external" | wc -l' % pip) == '0':
		subprocess.call([pip, 'install', '-r', 'pip_requirements.txt'])
	else:
		subprocess.call([pip, 'install', '--allow-all-external', '--allow-unverified', 'PIL', '--allow-unverified', 'PyXML', '-r', 'pip_requirements.txt'])
